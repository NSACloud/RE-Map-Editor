...
class OBJECT_PT_RE_MapTools_Object_Panel(Panel):
	...
	def draw(self, context):
		...
		layout = self.layout
		layout.prop(re_rsz_toolpanel,"game")
		...
		layout.operator("re_rsz.increment_rsz_index")
...
class OBJECT_PT_RE_MapTools_Visibility_Panel(Panel):
	...
	def draw(self, context):
		...
		layout.prop(re_rsz_toolpanel,"showDisplayMeshes")
		layout.prop(re_rsz_toolpanel,"showCollisions")
		layout.prop(re_rsz_toolpanel,"collisionWireframe")

class OBJECT_PT_RE_MapTools_RE4_Panel(Panel):
	bl_label = "RE RSZ: RE4 Tools"
	bl_idname = "OBJECT_PT_re_map_tools_re4_panel"
	bl_space_type = "VIEW_3D"   
	bl_region_type = "UI"
	bl_category = "RE Map Tools"
	bl_context = "objectmode"
	@classmethod
	def poll(self,context):
		return context is not None and context.scene.re_rsz_toolpanel.game == "RE4"

	def draw(self, context):
		re_rsz_toolpanel = context.scene.re_rsz_toolpanel
		layout = self.layout
		layout.label(text="Item Tools")
		layout.separator()
		layout.operator("re_rsz.re4_add_drop_item")
		layout.operator("re_rsz.re4_edit_drop_item")
		layout.prop_search(re_rsz_toolpanel, "re4ItemSCNRoot",bpy.context.scene,"objects")
		layout.prop_search(re_rsz_toolpanel, "re4ItemUSRRoot",bpy.context.scene,"objects")
		layout.label(text="NPC Tools")
		layout.separator()
		layout.operator("re_rsz.re4_add_npc")
		layout.operator("re_rsz.re4_edit_npc")

class LIST_UL_RSZ_Pointers(bpy.types.UIList):

	def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
		split = layout.split()
		split.prop(item, "fieldName", text="", emboss=False, translate=False, icon='NONE')
		split.prop(item, "targetObject", text="", emboss=False, translate=False, icon='NONE')
...
class OBJECT_PT_RSZObjectPointersPanel(Panel):
	...
	def draw(self, context):
		...
		re_rsz_object = activeObj.re_rsz_object
		...
		col2.template_list("LIST_UL_RSZ_Pointers", "", re_rsz_object, "rszObjectPointers", re_rsz_object, "activeIndex")
...
class OBJECT_PT_RSZPresetPanel(Panel):
	bl_label = "RE RSZ: Presets"
	bl_idname = "OBJECT_PT_rsz_presets_panel"
	bl_space_type = "VIEW_3D"   
	bl_region_type = "UI"
	bl_category = "RE Map Tools"   
	bl_context = "objectmode"
	...
	def draw(self, context):
		layout = self.layout
		scene = context.scene
		re_rsz_toolpanel = scene.re_rsz_toolpanel
		layout.operator("re_rsz.save_selected_as_preset")
		layout.operator("re_rsz.open_preset_folder")
		#layout.menu(OBJECT_MT_CustomMenu.bl_idname, text="Chain Group")
		layout.label(text="RSZ Presets")
		layout.prop(re_rsz_toolpanel, "rszPresets")
		layout.operator("re_rsz.add_rsz_preset")